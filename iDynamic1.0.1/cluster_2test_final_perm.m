function [sumt, sig_time,max_z,time_pn2]= cluster_2test_final_perm(data_wd,n1,n2,bootnum,p_value)
%% n1是第一组数据的终点行,n2是第几列数据
'ind_perm'
%先算原样本的t值
data_wd=data_wd(:,n2:end);
difmap=mean(data_wd(1:n1,:))-mean(data_wd(n1+1:end,:));
%boot
difmap_b=[];
for j=1:bootnum
    id_b=randsample(size(data_wd,1),size(data_wd,1),false);
    data_wdb=data_wd(id_b,:);
    difmap_b(j,:)=mean(data_wdb(1:n1,:))-mean(data_wdb(n1+1:end,:));
end

difmap_z=(difmap-mean(difmap_b,1))./std(difmap_b,[],1);
